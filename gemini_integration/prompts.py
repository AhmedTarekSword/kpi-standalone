...
def build_performance_analysis_prompt(
        ...
        "properties": {
            "english": {
                ...
                "description": (
                    ...
                    "data on the results, corroborated by broader contextual knowledge of similar events in "
                    "the same sector or period; if none are listed, explicitly state that no external events "
                    "were noted but note any relevant sector developments that may have affected this type of KPI.\n"
                    "4. Data Consistency & Availability: Assess whether the data provided is internally "
                    "consistent (e.g. period actuals align with stated period performance ratios, annual "
                    "figures are coherent with aggregated period values, no missing critical periods), "
                    "complete (all required periods and fields are populated), and reliable for drawing "
                    "meaningful conclusions; flag any gaps, anomalies, or data quality concerns explicitly.\n"
            ...
            "arabic": {
                ...
                "description": (
                    ...
                    "مدعومةً بمعرفة سياقية أوسع بالأحداث المشابهة في القطاع أو الفترة ذاتها؛ "
                    "وإذا لم توجد أحداث مُدرجة، يُصرَّح بذلك مع الإشارة إلى أي تطورات قطاعية "
                    "قد تكون أثّرت في هذا النوع من المؤشرات.\n"
                    "4. اتساق البيانات وتوفرها: تقييم مدى اتساق البيانات المقدمة داخليًا (مثل "
                    "تطابق القيم الفعلية للفترات مع نسب الأداء المحسوبة، وتوافق الأرقام السنوية مع "
                    "مجاميع الفترات، وغياب فترات جوهرية مفقودة)، واكتمالها (تعبئة جميع الفترات "
                    "والحقول المطلوبة)، وموثوقيتها لاستخلاص استنتاجات ذات معنى؛ "
                    "مع الإشارة صراحةً إلى أي ثغرات أو شذوذات أو مخاوف تتعلق بجودة البيانات.\n"
    ...
    user_msg = (
        ...
        "  - Anchor every statement to specific period figures and numeric values from the supplied data.\n"
        "  - CRITICAL: NEVER write internal technical field names or JSON variable names "
        "(such as 'period_actual', 'external_event_impact_percentage', 'period_performance', "
        "'annual_actual', 'benchmark_value', etc.) in the output text. "
        "Use plain, reader-friendly descriptions instead (e.g. 'Q1 actual result', 'estimated external event impact', 'annual performance ratio').\n"
        "  - Clearly distinguish between period-level and annual-level metrics when both exist.\n"
        "  - For the External Events Impact sub-section, draw on your knowledge of real-world sector "
        "trends and events (not just what is listed) to enrich the analysis.\n"
...
def build_root_causes_prompt(
    ...
    user_msg = (
        ...
        "  - Each cause MUST include a specific evidence reference — cite the period, the human-readable "
        "metric name (e.g. 'Q1 actual result', 'annual target', 'external event impact'), "
        "or exact figure from the data. Generic or unsupported causes are unacceptable.\n"
        "  - CRITICAL: NEVER expose internal technical field names or JSON variable names "
        "(such as 'period_actual', 'external_event_impact_percentage', 'period_performance', "
        "'annual_actual', 'benchmark_value', etc.) in the output text. "
        "Always replace them with plain, reader-friendly descriptive language.\n"
